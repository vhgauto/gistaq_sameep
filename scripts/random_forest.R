...
# t1 <- max(t_p2$.pred) - max(t_p2$.pred) %% 100 + 100
# # límite eje horizontal máximo, fecha más actual
# fe2 <- ceiling_date(max(t_p2$fecha), "month")
...
#     mutate(año = year(rango)) |>
#     group_by(año) |>
#     mutate(año1 = if_else(row_number() == 1, as.character(año), "")) |>
...
# g1 <- ggplot(data = t_p2,
#        aes(x = fecha, y = .pred)) +
#   geom_point(color = "#FC8D59", size = 1, alpha = .8, shape = 4) +
#   stat_smooth(geom = "line", alpha = 1, color = "#5E4FA2",
...
#   labs(y = "Turbidez (NTU)",
#        x = NULL,
#        title = "Estimación de <span style='color:darkblue'>turbidez</span> 
#        del agua en el ingreso a **La Toma**",
#        subtitle = glue("Un año de datos. Actualizado al 
#        {format(fecha_turb, '%d/%m/%y')}"),
#        caption = glue("Las <span style='color:#FC8D59'>**cruces en naranja**
#        </span> son los valores individuales, la 
#        <span style='color:#5E4FA2'>**línea en azul**</span> es la tendencia 
#        global.<br><span style='color:lightgrey'>{fyh}</span>"),
#        color = NULL) +
...
# ggsave(plot = g1,
#        filename = "figuras/gg_pred_001.png",
#        width = 14.5,
#        height = 10,
#        units = "cm",
#        dpi = 300)
